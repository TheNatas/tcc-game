...
const TIME_TO_WAIT_FOR_PLAYER_REACTION = 5.0
...
func start_next_level():
	notes_switches_on_current_level = 0
	right_notes_on_current_level = 0
...
func finish_level() -> void:
	var right_notes_percent = (float(right_notes_on_current_level) / MAX_NUMBER_OF_NOTES_SWITCHES_PER_LEVEL) * 100
...
func _on_sound_playing_degree_changed(current_playing_degree: int) -> void:
	notes_switches_on_current_level += 1
	if notes_switches_on_current_level == MAX_NUMBER_OF_NOTES_SWITCHES_PER_LEVEL:
		await get_tree().create_timer(TIME_TO_WAIT_FOR_PLAYER_REACTION).timeout
		finish_level()
		return
	await get_tree().create_timer(TIME_TO_WAIT_FOR_PLAYER_REACTION).timeout
	var player = get_tree().get_first_node_in_group("player")
	player_current_degree = player.is_at_current_degree
	...
	if current_playing_degree == player_current_degree:
		right_notes_on_current_level += 1
